...
export default function Description() {
  ...
  const handleSubmitComment = async () => {
    try {
      const docRef = await addDoc(
        collection(fireStore, "comments", subCategory, "topicComments"),
        newComment
      );
        ...
        <>
          ...
          {products.map((product) => (
            <article key={product.id} className="product-article">
              ...
              {product.fileURL &&
                product.fileURL.length > 0 &&
                product.fileURL.map((file, fileIndex) => (
  ...
  <div className="comment-form row">
    ...
    <div className="col-12">
      <button onClick={handleSubmitComment} className="submit-btn btn btn-primary">
        ...
        {comment.replies && comment.replies.length > 0 && (
          <div className="replies">
            {comment.replies.map((reply, idx) => (
              <p key={idx} className="reply-text">
                {reply}
              </p>
            ))}
          </div>
        ...
        {replyingToIndex === index && (
          <div className="reply-form">
            <textarea
              ...
              rows="3"
              className="reply-textarea form-control"
            />
            <button
